...
int main(){
    ...
    if(seq.size()!=p) {
        ...
        cout<<"The sequence is safe,No deadlock\n";
        cout<<"safe sequence:"<<endl;
        for(int i=0;i<p;i++) {
            if(i==p-1){
                  cout<<"P"<<seq[i]+0; //change to 1 if want to do 1 based indexing
            }
            else cout<<"P"<<seq[i]+0<<"-->";
        }
    }
