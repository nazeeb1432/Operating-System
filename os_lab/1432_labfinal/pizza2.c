...
sem_t pizza;    
sem_t deliver;
sem_t mutex;//for number_slices
sem_t order;
 int order_flag = true;//flag , fotr the first student to discover num_slices=0
...
void *student(void *arg){
    ...
    while(1){
        ...
        // sem_post(&mutex);
        // sem_wait(&mutex);
        if(num_slices == 0){//first student to discover num_slice=0 will deliver
